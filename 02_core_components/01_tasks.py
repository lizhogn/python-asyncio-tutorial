"""
02_core_components/01_tasks.py

Task 对象

Task 是 asyncio 中表示协程执行状态的对象，它是对协程的包装。
Task 对象提供了更丰富的功能，如取消、状态查询、异常处理等。
...
async def task_status_demo():
    ...
    # print(f"Task 名称: {task.get_name()}")  # Python 3.8+ 才支持
...
# 7. Task 的优先级和调度
async def task_priority_demo():
    """演示 Task 的优先级和调度"""
    print("=== Task 优先级演示 ===")
    
    async def priority_task(name, priority, duration):
        """带优先级的任务"""
        print(f"[{datetime.now().strftime('%H:%M:%S')}] {name} (优先级:{priority}) 开始")
        await asyncio.sleep(duration)
        print(f"[{datetime.now().strftime('%H:%M:%S')}] {name} 完成")
        return f"{name} 结果"
    ...
    # 创建不同优先级的任务
    tasks = [
        asyncio.create_task(priority_task("高优先级", "高", 1)),
        asyncio.create_task(priority_task("中优先级", "中", 1)),
        asyncio.create_task(priority_task("低优先级", "低", 1)),
    ]
